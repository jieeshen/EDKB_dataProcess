...
public class createMolStructTab_v2 {
    final public static String EXCELFILE="C:\\Documents and Settings\\jshen\\My Documents\\Research\\EDKB\\ER\\ERDB\\Assays_120613.xls";
...
//        ArrayList notFoundList=readList("C:\\Temp_data2\\notFoundinEMBL.txt");
//        if (notFoundList.contains(inchikey)) {
//            return "not found";
//        }
        if (addedMolStruct.size()>0){
            for(String[] al:addedMolStruct ){
                if (al[3].equals(inchikey)){
                    System.out.print(" ALREADYHAS "+al[0]);
                    return al[0];
                }
            }
        }
...
//        System.out.println(mappingList.get("647"));
        ...
        for (int i=1;i<matrixSize;i++){
//        for (int i=1;i<11;i++){
            System.out.print(Integer.toString(i)+" "+outArray[i][2]+" "+outArray[i][3]+" ");
            if (outArray[i][3] != "") {
                addMol(outArray[i][3]);
            } else {
                if (outArray[i][10]!=""){
...
//                    System.out.println(molName);
                        if (outArray[i][11].trim().equals(molName)){
//                        System.out.println("got it "+mol.getName()+" "+outArray[i][5]);
                            ...
                            if (molinchikey.equals(outArray[i][14]) && outArray[i][15]!=""){
                                System.out.print(" found in table");
                                outArray[i][3]=outArray[i][15];
                                addMol(outArray[i][3]);
                                break;        // added on 6/13/2012, found there are duplicated structure in the sdf file, so it added more than once and
                            } else{
