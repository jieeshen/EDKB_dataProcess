 ...
 */
public class writeSdfFromDb_v4 {
    final public static String MOLLISTFILE="C:\\Temp_data2\\sorted_molregno.txt";
    final public static String OUTPUTSDFILE="C:\\Temp_data2\\erdb_12_symyx_compact.sdf";
    final public static String driver="com.mysql.jdbc.Driver";
    final public static String url="jdbc:mysql://localhost:3306/erdb_12";
...
//        for(int i=72;i<=74;i++){
            ...
            while(rsA.next()){
                ...
                int refsection=(i_activity-1)/200;
                descPool[refsection]=descPool[refsection]+"["+Integer.toString(i_activity)+"] "+reference;
            ...
            for (int ic=0;ic<3;ic++){
                if (descPool[ic]!=""){
                    m.setProperty("activity_references"+ic,descPool[ic]);
                }
            }
            System.out.printf("%d: %d %d %d %d %d %d %d %d %d %d %d\n",i,b_a_f.size(),b_a_l.size(),b_a_u.size(),
                    b_b_f.size(),b_b_l.size(),b_b_u.size(),b_u.size(),r_a.size(),r_b.size(),r_u.size(),c.size());
            me.write(m);
